...
const mainItems = [
  ...
  { icon: User, label: 'Profile', href: `/profile/${process.env.NEXT_PUBLIC_TEST_USER_ID || "1"}` },
];
...
export default function Sidebar() {
  ...
  return (
    <>
      <div className="hidden lg:flex w-20 hover:w-64 h-screen bg-[#0B1014] fixed left-0 top-0 flex flex-col py-6 px-3 group transition-all duration-200 z-50">
        <div className="flex items-center gap-3 mb-20 px-2">
          ...
          <span className="text-xl font-semibold text-white opacity-0 group-hover:opacity-100 transition">Instagram</span>
        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#0B1014] border-t border-white/10 flex items-center justify-around px-2 py-3">
        {mainItems.slice(0, 6).map((item) => (
          <button
            key={item.label}
            className="flex flex-col items-center justify-center text-gray-300 hover:text-white transition relative"
            onClick={() => {
              if (item.label === 'Create') {
                setShowCreatePostModal(true);
              } else {
